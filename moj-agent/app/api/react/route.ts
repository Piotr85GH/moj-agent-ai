...
Jestes autonomicznym agentem. Gdy dostajesz ZADANIE (nie pytanie), MUSISZ je zrealizowac krok po kroku.

## TWOJ PROCES:

Dla KAZDEGO kroku wypisz:

### 🧠 Mysle...
Co musze teraz zrobic? Jakie informacje mi brakuje? Ktorego narzedzia uzyc?
...
### 👁️ Obserwuje...
Co dostalem? Czy to wystarczy do odpowiedzi? Jesli nie - jaki nastepny krok?

Powtarzaj az bedziesz miec WSZYSTKO co potrzebne.

Na koniec:

### ✅ Wynik koncowy
Podaj pelna, konkretna odpowiedz oparta na zebranych danych. Cytuj zrodla (API, Wikipedia, Google).

## ZASADY:
- ZAWSZE pokazuj tok pracy w naglowkach markdown: "### 🧠 Mysle...", "### 👁️ Obserwuje...", "### ✅ Wynik koncowy".
- Pokazuj krotki, uzytkowy plan kroku i obserwacje z danych; nie pokazuj prywatnego rozumowania.
- NIE zgaduj - jesli potrzebujesz danych, UZYJ narzedzia.
- Maksymalnie 5 glownych krokow.
- Jesli narzedzie zwroci blad - sprobuj inaczej lub poinformuj.
- LACZ dane z wielu narzedzi w spojna odpowiedz.
- Gdy korzystasz z internetu albo API, podawaj nazwe zrodla i URL, jesli narzedzie go zwrocilo.

## BAZA WIEDZY FIRMY
- Masz dostep do bazy wiedzy firmy przez narzedzie searchKnowledge.
- Gdy uzytkownik pyta o ceny, pakiety, oferty, regulamin, warunki, procedury albo FAQ, ZAWSZE najpierw uzyj searchKnowledge.
- Odpowiadaj TYLKO na podstawie znalezionych fragmentow. Nie wymyslaj cen, warunkow ani szczegolow oferty.
- Jesli baza wiedzy nie zawiera odpowiedzi, powiedz: "Nie mam tej informacji w bazie wiedzy. Skontaktuj sie z firma."
- Gdy odpowiadasz na podstawie bazy wiedzy, ZAWSZE dodaj na koncu: "📎 Źródło: [tytuł] (dodano: [added_at])" albo "📎 Źródła: [tytuł 1] (dodano: [added_at]), [tytuł 2] (dodano: [added_at])". Jesli added_at jest puste, podaj sam tytul.
- Gdy searchKnowledge zwroci 0 wynikow albo wyniki nie odpowiadaja na pytanie, NIE odpowiadaj z wiedzy ogolnej. Powiedz: "Nie mam informacji na ten temat w mojej bazie wiedzy. Skontaktuj sie z firma bezposrednio."
- Pytania o firme/cennik/FAQ -> searchKnowledge najpierw. Pytania ogolne -> inne narzedzia. Obliczenia -> calculator.
...
async function readWebPage(url: string) {
  try {
    const response = await fetchWithTimeout(url, {
      headers: {
        "user-agent":
          "Mozilla/5.0 (compatible; MojAgentReAct/1.0; +https://localhost)",
      },
    });
...
async function searchWikipedia(query: string, language: string) {
  ...
  try {
    response = await fetchWithTimeout(url, {
      headers: {
        "api-user-agent": "MojAgentReAct/1.0 (local workshop)",
      },
    });
  } catch (error) {
...
function getNotesStore() {
  const store = globalThis as typeof globalThis & { __reactNotes?: Note[] };
  store.__reactNotes ??= [];
  return store.__reactNotes;
}
      ...
      if (!reader) {
        const result = streamText({
          ...
          tools: {
            calculator: tool({
              description:
                "Wykonuje obliczenia matematyczne, procenty, VAT, netto/brutto i przeliczenia.",
              inputSchema: z.object({
                expression: z.string().describe("Wyrazenie, np. 5000 * 4.25."),
              }),
            ...
            currentDateTime: tool({
              description:
                "Zwraca aktualna date i czas w strefie Europe/Warsaw.",
              inputSchema: z.object({}),
              execute: async () => ({
            ...
            getExchangeRate: tool({
              ...
              inputSchema: z.object({
                from: z.string().describe("Waluta zrodlowa, np. EUR."),
                to: z.string().describe("Waluta docelowa, np. PLN."),
            ...
            getHolidays: tool({
              description:
                "Pobiera swieta publiczne dla kraju i roku z Nager.Date.",
              inputSchema: z.object({
                countryCode: z
                  .string()
                  .default("PL")
                  .describe("Kod kraju ISO 3166-1 alpha-2, np. PL, DE, FR."),
                year: z
                  .number()
                  .int()
                  .min(2000)
            ...
            saveNote: tool({
              description:
                "Zapisuje notatke w pamieci procesu serwera dla tej sesji dev.",
              inputSchema: z.object({
            ...
            getNotes: tool({
              description: "Zwraca notatki zapisane przez saveNote.",
              inputSchema: z.object({}),
